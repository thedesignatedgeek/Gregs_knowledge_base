...
# In Progress - support delete topic
...
#       Set Scrolledlistbox background to cornsilk2
#       set Scrolledtextbox background to cornsilk2, selectbackground to skyblue2
#       startup - place_forget for btnRemove, btnTopicEdit on buttonbar
#       database has 22 records now
#       When in Show All Topics screen, topic records are returned in
#          ascending order.
...
def main(*args):
    ...
    _top1 = root
    _w1 = gkb.Main(_top1)
    # Runs the startup function before showing the Toplevel form.
    startup()
    root.mainloop()
...
def startup():
    ...
    _w1.btnRemove.place_forget()
    _w1.btnTopicEdit.place_forget()
...
def on_keywordSearch(*args):
    ...
    if shared.kwdMode == "kwd":
        ...
        # msg = "Topic search is not ready yet!"
        # messagebox.showinfo(programName, msg, parent=_top1)
        topic = _w1.EntryKeyword.get()
        returns = queryTopic_search(topic)
        if _debug4:
            print(returns)
            print(f"{len(returns)=}")
        if returns:
            if len(returns) == 1:
                ...
                if _debug4:
                    print(returns[0][2])
                textbox.insert(1.0, returns[0][2])
                _w1.FrameMainText.lift()
            elif len(returns) > 1:
                messagebox.showinfo(
                    programName,
            ...
            messagebox.showinfo(
                programName, "No records found!", parent=_top1, icon=messagebox.INFO
            )


def get_keywords(kwd):
    global connection, cursor
    kwdquery = f"""SELECT info.pkid, info.topic, keywordsmain.KeywordText, info.code, keywords.idKeywords, keywords.KeywordsMain FROM info INNER JOIN keywords ON info.pkid = keywords.InfoMain INNER JOIN keywordsmain ON keywords.KeywordsMain = keywordsmain.idKeywordsMain WHERE topic LIKE "%{kwd}%" and info.Active=1"""
    if _debug4:
        print(kwdquery)
    resp = cursor.execute(kwdquery)
    return resp
...
def on_btnHelp(*args):
    if _debug:
        ...
        for arg in args:
            print("    another arg:", arg)
        sys.stdout.flush()
    pth = os.path.join(location, "docs", "html", "index.html")
    webbrowser.open_new(pth)
    # titl = programName
    # msg = (
...
def on_SQLlistboxSelect(*args):
    # print("Into on_SQLlistboxSelect")
    # print("FrameSQL")
    if _debug3:
...
def setup_textbox():
    ...
    for textbx in tblist:
        textbx.configure(background="cornsilk2")
        textbx.configure(selectbackground="skyblue2")
...
def on_btnKwdClear(*args):
    if _debug:
        print("gkb_support.on_btnKwdClear")
        for arg in args:
            print("    another arg:", arg)
        sys.stdout.flush()
    _w1.EntryKeyword.set("")


def on_btnSwap(*args):
    if _debug:
        print("gkb_support.on_btnSwap")
        for arg in args:
            print("    another arg:", arg)
        sys.stdout.flush()
    if shared.kwdMode == "kwd":
        _w1.KwdTopic.set("Topic: ")
        shared.kwdMode = "Topic"
    else:
        _w1.KwdTopic.set("Keywords: ")
        shared.kwdMode = "kwd"
...
WHERE
    ...
    if recs:
        ...
        if len(recs) == 0:
            titl = programName
            msg = f"Sorry, no records found for {topic}"
            ico = messagebox.INFO
            messagebox(titl, msg, parent=_top1, icon=ico)
        elif len(recs) == 1:
            return recs
        elif len(recs) > 1:
            if _debug4:
...
FROM
    ...
    INNER JOIN keywordsmain
     ON keywords.KeywordsMain = keywordsmain.idKeywordsMain
WHERE
    KeywordText LIKE "%{keywd}%" and info.Active=1"""
